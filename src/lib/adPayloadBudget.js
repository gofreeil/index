	...
	for (let round = 0; round < 40; round++) {
		...
		const push = (cur, floor, png, apply) => {
			if (typeof cur === 'string' && cur.startsWith('data:image/'))
				imgs.push({ cur, floor, png, apply });
		};
		push(payload.mainImage, MIN_MAIN_CHARS, false, (v) => {
			payload.mainImage = v;
		});
		push(payload.landing?.image, MIN_SIDE_CHARS, false, (v) => {
			payload.landing.image = v;
		});
		for (const p of payload.landing?.products ?? [])
			push(p.image, MIN_SIDE_CHARS, false, (v) => {
				p.image = v;
			});
		push(payload.logo, MIN_LOGO_CHARS, true, (v) => {
			payload.logo = v;
		});
