 ...
 *
 * מיוצאות גם למנוע הצעת התגיות ($lib/tagSuggest): אותן מילים בדיוק הן מה
 * שכדאי לבעל העסק לרשום על עצמו, ושם הן חוסכות לגולש את מסלול ההצעות.
 */
...
].map((g) => [...new Set(g.map(norm).filter(Boolean))]);
