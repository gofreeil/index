<script>
	...
	/* הרצפה של המסגור: מה שנראה תמיד על המפה, גם כשכל הנקודות מצטופפות
	   בגוש דן. מהגבול הצפוני (33.3) ועד צפון הנגב (30.5) — כל החלק המיושב
	   של הארץ. בלי הערבה ואילת: הן שליש נוסף של אורך המדינה וכמעט בלי
	   עסקים, והכללתן הייתה מקטינה את כל השאר בשליש כדי להראות שטח ריק.
	   מערב-מזרח (34.3—35.65) כמעט לא משפיע: המסגרת גבוהה וצרה, והגובה
	   הוא שקובע את רמת הזום. */
	...
	function render() {
		...
		// גם אין יותר בליטת זום אחרי המסגור: היא נועדה להדק מסגור רופף
		// סביב הנקודות, ועכשיו המסגרת היא קופסה מוצהרת — הידוק שלה רק
		// היה מקצץ מקצות הארץ.
		map.fitBounds(fit, {
			paddingTopLeft: [4, 34],
