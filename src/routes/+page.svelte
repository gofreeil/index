				...
				<div class="mb-8 space-y-4">
					<div class="relative">
						<input
							...
							aria-label={t.search}
							class="w-full rounded-xl border border-gray-700 bg-gray-800 px-4 py-3 pr-12 text-gray-100 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-900/30"
						/>
						<svg
					...
					<div class="flex flex-wrap items-center gap-3">
						...
						{#if searchTerm || selectedCategory !== 'all' || selectedLocation !== 'all'}
							<button onclick={clearFilters} class="text-sm font-medium text-gray-400 hover:text-blue-400">
								ביטול כל המסננים
							</button>
						{/if}
					</div>
		...
		<aside class="hidden w-64 shrink-0 lg:block">
			<div class="sticky top-32 space-y-6">
				<a
					...
					class="group relative flex h-40 flex-col items-center justify-center overflow-hidden rounded-2xl shadow-md"
				>
					<img
						src={sidebarBanner}
						alt=""
					...
					<div class="relative z-10 p-4 text-center">
						<span class="block text-lg font-bold text-white">בעל עסק?</span>
						<p class="mt-1 text-xs font-medium text-white/90">הצטרפו למדריך בעלי המקצוע</p>
					</div>
				</a>

				<!-- מודעות קהילה (מאותו מקור נתונים כמו המובייל) -->
				<div class="space-y-4">
					{#each ads.slice(0, 4) as ad (ad.id)}
						<a
							href={ad.href}
							target="_blank"
							rel="noopener"
							class="group relative block overflow-hidden rounded-xl shadow-sm transition hover:shadow-md"
						>
							<img
								src={ad.image}
								alt={ad.title}
							...
							<div class="absolute right-3 bottom-3 left-3 text-right">
								<h4 class="text-sm font-extrabold text-white drop-shadow">{ad.title}</h4>
								<p class="mt-0.5 text-[11px] text-white/90 drop-shadow">{ad.description}</p>
							</div>
						</a>
					{/each}
				</div>
