	...
	try {
		...
		if (rawImages) {
			bannerArray = rawImages
				.split(',')
				.map((/** @type {string} */ url) => getDirectImageUrl(url.trim()));
		}
		...
		const rawYoutube =
			findValue(business, 'YouTube') ||
			findValue(business, 'יוטיוב') ||
			findValue(business, 'סרטון');
		...
		const formattedBusiness = {
			...
			salesArea: cleanText(
				business['אזור מכירה ארצי (אנטרנטי). אם האיזור פרטי נא לפרט היכן'] || ''
			),
