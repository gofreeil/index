...
export const actions = {
	save: async ({ request, params, locals }) => {
		...
		invalidateMatches();
		// אחרי שמירה מוצלחת חוזרים לכרטיסייה עצמה — זה מה שבעל העסק בא לראות,
		// והוא רואה מיד את התוצאה במקום להישאר בטופס עם שורת "נשמר". ?saved=1
		// מציג שם אישור קצר.
		throw redirect(303, `/business/${params.id}?saved=1`);
	}
};
