...
<main class="mx-auto max-w-3xl px-4 py-6 sm:px-6">
	...
	<!-- ── דרישת בעלות ──────────────────────────────────────────
	     ...
	     ב-92 הדפים. -->
	{#if form?.claimedNow || (claim.open && (!data.isAdmin || claim.matchedBy))}
		<section
			id="claim"
		...
		>
			{#if form?.claimedNow}
				<p class="text-base text-emerald-400">
					✓ {t.claimDoneNow}
					<a href="/business/{business.documentId}/edit" class="font-medium text-blue-400 hover:text-blue-300">{t.editCard}</a>
				</p>
			{:else if claimSent && !claim.instant}
				<p class="text-base text-emerald-400">
			...
			{:else}
				...
				{#if claim.instant}
					<!-- הקישור החתום מה-SMS — בעלות בלחיצה, בלי הערה ובלי אדמין -->
					<form
						method="POST"
						action="?/claim"
						use:enhance={() => {
					...
					>
						<button
							...
							class="mt-3 rounded-lg bg-blue-600 px-4 py-2 text-base font-semibold text-white transition hover:bg-blue-500 disabled:opacity-40"
						>
							{claimSending ? '…' : t.claimBtn}
						</button>
					</form>
				{:else if claimOpen}
					<form
	...
	<!-- הסיווג — השורה האחרונה של הכרטיסייה עצמה, ממש מעל חוות הדעת ובגוון
	     עמום: הוא מיון של המדריך ולא פרט שהמבקר מחפש, ולכן אינו קוטע שום
	     מדור. עומד בפני עצמו ולא בתוך פרטי הקשר — עסק בלי כתובת ובלי אזור
	     שירות עדיין מציג את הסיווג שלו. -->
	{#if catLine}
		<p class="mt-6 text-sm text-gray-500">{catLine}</p>
	{/if}
