<script>
	...
	// ===== המרת גרדיאנט: CSS (תצוגת הבילדר) → מחלקות Tailwind =====
	// האוסף המשותף submitted-ads שומר גרדיאנטים כמחלקות Tailwind
	// (bg-gradient-to-br {gradient} בסרגל הצד ובדף /ads/[id]), ולכן
	// בשליחה ממירים. לשמור מסונכרן עם palettes שבבילדר ועם $lib/adGradients.js.
	/** @type {Record<string, string>} */
	const CSS_TO_TW = {
		'linear-gradient(135deg, #f59e0b, #ea580c)': 'from-amber-500 to-orange-600',
		'linear-gradient(135deg, #f97316, #ef4444)': 'from-orange-500 to-red-500',
		'linear-gradient(135deg, #facc15, #f59e0b)': 'from-yellow-400 to-amber-500',
		'linear-gradient(135deg, #dc2626, #db2777)': 'from-red-600 to-pink-600',
		'linear-gradient(135deg, #f43f5e, #c026d3)': 'from-rose-500 to-fuchsia-600',
		'linear-gradient(135deg, #be123c, #7f1d1d)': 'from-rose-700 to-red-900',
		'linear-gradient(135deg, #d946ef, #9333ea)': 'from-fuchsia-500 to-purple-600',
		'linear-gradient(135deg, #9333ea, #db2777)': 'from-purple-600 to-pink-600',
		'linear-gradient(135deg, #7c3aed, #4338ca)': 'from-violet-600 to-indigo-700',
		'linear-gradient(135deg, #4f46e5, #2563eb)': 'from-indigo-600 to-blue-600',
		'linear-gradient(135deg, #2563eb, #0891b2)': 'from-blue-600 to-cyan-600',
		'linear-gradient(135deg, #38bdf8, #3b82f6)': 'from-sky-400 to-blue-500',
		'linear-gradient(135deg, #14b8a6, #0891b2)': 'from-teal-500 to-cyan-600',
		'linear-gradient(135deg, #10b981, #0f766e)': 'from-emerald-500 to-teal-700',
		'linear-gradient(135deg, #16a34a, #059669)': 'from-green-600 to-emerald-600',
		'linear-gradient(135deg, #a3e635, #22c55e)': 'from-lime-400 to-green-500',
		'linear-gradient(135deg, #64748b, #374151)': 'from-slate-500 to-gray-700',
		'linear-gradient(135deg, #1f2937, #0f172a)': 'from-gray-800 to-slate-900',
		'linear-gradient(135deg, #fdba74, #f472b6)': 'from-orange-300 to-pink-400',
		'linear-gradient(135deg, #6ee7b7, #2dd4bf)': 'from-emerald-300 to-teal-400',
		'linear-gradient(135deg, #eab308, #b45309)': 'from-yellow-500 to-amber-700',
		'linear-gradient(135deg, #334155, #1e3a8a)': 'from-slate-700 to-blue-900'
	};
	/** @param {string} css */
	function gradientTw(css) {
		return CSS_TO_TW[css] ?? 'from-amber-500 to-orange-600';
	}
	...
	function buildPayloadSnapshot() {
		return {
			...
			hoverText,
			cta,
			// האוסף המשותף מצפה למחלקות Tailwind, לא למחרוזת ה-CSS של התצוגה
			gradient: gradientTw(gradient),
			logo,
			mainImage,
			mainImageFit,
	...
	function applyPayloadImages(p) {
		...
		try {
			...
			localStorage.setItem(
				LS_KEY,
				JSON.stringify({ ...cur, logo: p.logo, mainImage: p.mainImage })
			);
		} catch {}
	}
	...
	async function processImageFile(file, target) {
		...
		if (!fit.ok) {
			uploadIssue = {
				...
				msg:
					`אין מקום לתמונה הזו — המודעה כולה מוגבלת במשקל, וגם אחרי דחיסה היא חורגת. ` +
		...
		if (res.wasCompressed || fit.changed) {
			const finalUrl =
				target === 'landing'
					...
					: (candidate.landing.products[products.findIndex((p) => p.id === target.id)]?.image ??
						url);
	...
	function addProduct() {
		products = [
			...products,
			{ id: nextProductId++, name: '', price: '', image: '', description: '' }
		];
	}
	/** @param {number} id */
	...
	function setPayDuration(days) {
		payDuration = days;
		try {
			localStorage.setItem(PLAN_DAYS_KEY, String(days));
		} catch {}
	}
	/** @param {SubmitEvent} e */
	...
	onMount(() => {
		...
		// שחזור התקופה שנבחרה בביקור קודם - שתישאר עקבית עם הבילדר
		const storedPlanDays = Number(localStorage.getItem(PLAN_DAYS_KEY));
		if (storedPlanDays === 180) payDuration = 180;
	...
	let canSubmit = $derived(fieldsOk && !overweight && loggedIn);
	...
	async function submitAd() {
		...
		try {
			...
			if (!(await shrinkAdPayload(payload, AD_BODY_LIMIT)).ok) {
				throw new Error(
					'התמונות כבדות מדי לשליחה אחת - הקטינו את התמונה הראשית או תמונות המוצרים ונסו שוב'
				);
			}
	...
	// שהמפרסם לא יגלה אחרי ההתחברות שהתוכן נעלם. הניווט הוא טעינה מלאה
	// כי דף ההתחברות מחזיר לעמוד הקודם לפי document.referrer.
	...
	{#if accessGranted}
		{#if compressNotice.visible}
			<div class="compress-toast" role="status" aria-live="polite">
				<button
					type="button"
					class="compress-toast-close"
					onclick={dismissCompressNotice}
					aria-label="סגור">✕</button
				>
				<p>
					🪄 התמונות כווצו אוטומטית מ-{fmtWeight(compressNotice.originalBytes)} ל-{fmtWeight(
						compressNotice.finalBytes
					)} כדי שהמודעה תעמוד במשקל המותר
				</p>
			</div>
		{/if}
		...
		<header class="l-header">
			...
			<p>
				כשגולש לוחץ על הפרסומת שלכם - הוא מגיע לדף נחיתה אישי עם כל הפרטים על העסק. כאן ממלאים את
				התוכן שלו.
			</p>
		...
		{#if !submitted}
				...
				<div class="l-grid">
					...
					<div class="span-2">
						...
						<input
							id="l-headline"
							type="text"
							bind:value={landingHeadline}
							placeholder="למשל: פיצה של אמא - הפיצרייה של השכונה"
							class="text-input"
						/>
					</div>
					<div class="span-2">
						...
						<textarea
							...
							rows="3"
							placeholder="שניים-שלושה משפטים שמציגים את העסק"
							class="text-input"
						></textarea>
					</div>
					<div class="span-2">
						...
						<textarea
							id="l-extended"
							bind:value={landingExtended}
							rows="8"
							placeholder="ספרו על העסק בהרחבה..."
							class="text-input"
						></textarea>
					</div>
					...
					<div class="span-2">
						...
						<p class="field-note">
							כל יתרון מופיע בשורה משלו עם <strong class="amber-strong">✓</strong> - קצר וחד (עד 80 תווים).
						</p>
						<div class="adv-inputs">
							<input
								type="text"
								bind:value={landingAdvantages[0]}
								maxlength="80"
								placeholder="למשל: משלוח חינם מעל 100 ₪"
								class="text-input"
							/>
							<input
								type="text"
								bind:value={landingAdvantages[1]}
								maxlength="80"
								placeholder="למשל: חומרי גלם טריים כל בוקר"
								class="text-input"
							/>
							<input
								type="text"
								bind:value={landingAdvantages[2]}
								maxlength="80"
								placeholder="למשל: הנחה קבועה לחברי הקבוצה"
								class="text-input"
							/>
						</div>
					</div>
						...
						<div class="product-row">
							...
							>
								...
								<input
									type="file"
									accept="image/*"
									onchange={(e) => handleProductImage(e, p.id)}
									class="hidden-input"
								/>
							</label>
							<div class="product-fields">
								<input
									type="text"
									bind:value={products[idx].name}
									placeholder="שם המוצר"
									class="text-input small"
								/>
								<input
									type="text"
									bind:value={products[idx].price}
									placeholder="מחיר (₪)"
									class="text-input small"
								/>
								<input
									type="text"
									bind:value={products[idx].description}
			...
			<section class="step-card">
				...
				<textarea
					...
					rows="5"
					maxlength="500"
					placeholder="ספרו מה מיוחד בעסק שלכם..."
					class="text-input"
				></textarea>
				...
				<div class="l-grid">
					...
					<div class="span-2">
						...
						<input
							...
							type="text"
							bind:value={hours}
							placeholder="למשל: א'-ה' 9:00-19:00, ו' 8:00-13:00"
							class="text-input"
						/>
					</div>
				</div>
							...
							<div class="landing-hero-inner" class:has-media={!!(landingImage || mainImage)}>
								<div class="landing-hero-content">
									...
									<div class="landing-cta-row">
										...
										{/if}
										{#if whatsapp}
											<a href="https://wa.me/{whatsapp.replace(/\D/g, '')}" class="landing-cta wa"
												>וואטסאפ</a
											>
										{:else if website}
							...
							<section class="landing-section landing-contact">
								...
								{#if address || hours}
									<p class="landing-meta">
										{#if address}📍 {address}{/if}{#if address && hours}
											·
										{/if}{#if hours}🕒 {hours}{/if}
									</p>
								{/if}
							</section>
						{/if}
			...
			<section class="step-card">
				...
				<ul class="checklist">
					...
					<li class:done={!!(phone || website)}>
						<span>{phone || website ? '✅' : '⬜'}</span> דרך ליצירת קשר
					</li>
					<li class:done={products.length > 0}>
						<span>{products.length > 0 ? '✅' : '⬜'}</span> מוצרים ({products.length})
					</li>
				...
				<div class="pay-box">
					...
					<div class="pay-duration" role="group" aria-label="תקופת הפרסום">
						...
						<button
							...
							class:active={payDuration === 30}
							onclick={() => setPayDuration(30)}>חודש</button
						>
						<button
							type="button"
							class="pay-duration-btn"
							class:active={payDuration === 180}
							onclick={() => setPayDuration(180)}>חצי שנה</button
						>
					</div>
					{#if payCodeOk}
				...
				{#if !loggedIn}
					<div class="login-gate">
						...
						<p class="login-gate-text">
							הפרסומת נשמרת על החשבון שלכם, וכך תוכלו לעקוב אחריה, לערוך אותה בכל רגע ולחדש בסוף
							התקופה. בלי חשבון אין דרך להחזיר אתכם אליה.
						</p>
						<button
							type="button"
							class="login-gate-btn"
							onclick={goLoginToSubmit}
							disabled={leavingToLogin}
						>
				...
				{#if !fieldsOk || overweight}
					<p class="submit-note">
						{#if !fieldsOk}
							כדי לשלוח: תמונה, כותרת, סלוגן, טקסט ריחוף, דרך קשר וכותרת/פתיח לדף הנחיתה. חסר משהו?
							חזרו <button type="button" class="inline-link" onclick={goBack}>לעריכת הכרטיס</button
							>.
						{/if}
						{#if overweight}
				...
				<button
					...
					class="submit-btn"
					class:enabled={canSubmit && !submitting}
				>
...
<style>
	...
	.l-badge {
		...
		border-radius: 999px;
		background:
			linear-gradient(to right, rgba(245, 158, 11, 0.2), rgba(249, 115, 22, 0.2)), #16264d;
	...
	.step-card {
		background:
			linear-gradient(180deg, rgba(255, 255, 255, 0.03), rgba(255, 255, 255, 0.01)), #16264d;
		border: 1px solid #3b5794;
	...
	.upload-zone-product.has-image {
		border-style: solid;
	}
	.upload-zone-product img {
		width: 100%;
		height: 100%;
		object-fit: cover;
	}
	...
	.on-hero .advantage-text {
		color: rgba(255, 255, 255, 0.95);
	}
	.strip .advantage-text {
		color: #e5e7eb;
	}
	.products-grid {
		display: grid;
