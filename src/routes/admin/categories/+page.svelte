<script>
	...
	function removeRow(i) {
		...
		const impact = r.count
			? `\n${r.count} כרטיסיות מסווגות אליה — הן לא נמחקות: כל אחת תעבור מיד לתחום שמתאים לה לפי התוכן שלה (ומה שלא יימצא לו תחום — ל"אחר").`
			: '';
...
<main class="mx-auto max-w-4xl pb-10" dir="rtl">
	...
	>
		...
		<ul class="list-inside list-disc space-y-0.5">
			...
			<li>
				מחיקה (🗑) מסירה קטגוריה מהאתר ומהטפסים. הכרטיסיות שהיו בה לא נמחקות — הן מסווגות מיד מחדש
				לפי התוכן שלהן; מובנית שנמחקה ממתינה ברשימת שחזור.
			</li>
		</ul>
	</div>
		...
		{#if removedBuiltins.length}
			<div class="mt-4 rounded-2xl border border-dashed border-red-500/25 bg-red-950/10 p-4">
				<p class="mb-2 text-xs font-bold text-gray-400">
					🗑 קטגוריות מובנות שנמחקו — לא מוצגות באתר, בטופס ההגשה או בסיווג האוטומטי, והכרטיסיות
					שהיו בהן סווגו מחדש. שחזור מחזיר אותן ואת הכרטיסיות שלהן:
				</p>
